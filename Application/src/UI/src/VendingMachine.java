...
public class VendingMachine {
    ...
    public static final String Staff_Instruction =
            "\n\'fill\' to fill the VM\n\'print\' to print the transactions\n";

    public static ArrayList<String> list_of_Product_name = new ArrayList<String>(){{
        for (Product p : Product.values()) {
            add(p.nameProperty().get());
        }
    }};

    public static ArrayList<Integer> List_of_Product_id = new ArrayList<Integer>(){{
        for (Product p : Product.values()) {
            add(p.product_idProperty().get());
        }
    }};
    ...
    public VendingMachine(){
        ...
        for (Product p : Product.values()){ this.Products.put(p.nameProperty().get(), p); }
    }
    ...
    public HashMap<Product, Integer> getProduct_status(){
        HashMap<Product, Integer> map = new HashMap<>();
        for (Product p : this.Products.values()){ map.put(p, p.getNum_of_items()); }
        return map;
    }

    public String Product_statusUI(){
        return
                "------------------------------------\n" +
                "|     Drinks      |   Chocolates   |\n" +
                "------------------------------------\n" +
 ...
 String.format("|sweet_chillies %d |   part_mix %d   |\n", this.Products.get("sweet_chillies").getNum_of_items(), this.Products.get("part_mix").getNum_of_items()) +
    ...
    public static String money_reminder(){
    ...
    public static double convert_input_money(String input_money){
    ...
    public static ArrayList<String> Calculate_change(double change){
    ...
    public String user_add(List<String> ls){
            ...
            try{
                ...
                if (List_of_Product_id.contains(product_id)){
                    ...
                    for (String n : Products.keySet()){
                        ...
                        if (p.product_idProperty().get() == product_id){
                            product_name = n;
                        }
                    }
                    if (ls.size() == 2){
    ...
    public String user_delete(List<String> ls){
    ...
    public String user_receipt(){
        ...
        for(ProductSold e : t.getItem_sold()){
            System.out.println(e.getName() + " * " + e.getNum_sold());
            l_r += e.getName() + " * " + e.getNum_sold() + "\n";
        }
        System.out.println();
        l_r = l_r + "\n";
    ...
    public String user_check(List<String> ls){
        String l_r = "";
        if(ls.size()!=1){
            ...
            if (double_money == -1.0){
                ...
                Application.input_amount += double_money;
                if (Application.input_amount < total_price){
                    double need_more_money = (double)Math.round((total_price - Application.input_amount)*100)/100;//making it not with infinity digit
                    ...
                    double change = (double) Math.round((Application.input_amount - total_price)*100) / 100;
                    ...
                    ArrayList<ProductSold> temp = new ArrayList<>();
                    ...
                    Application.isInTransaction = false;
                }
            }
        }
        return l_r;
    ...
    public String user_cancel(){
        for (Map.Entry<Product, Integer> e : this.shoppingCart.getList_of_Products().entrySet()){
            if(e.getValue() > 0){
                this.Products.get(e.getKey().nameProperty().get()).add_back(e.getValue());
                this.shoppingCart.delete_Product(Products.get(e.getKey().nameProperty().get()), e.getValue());
            }
        }
    ...
    public String staff_print(){
        ...
        for (int i=0;i<transactions.size();i++){
            ...
            for(ProductSold p: current_t.getItem_sold()){
                System.out.println("    "+ p.getNum_sold() +" "+ p.getName() +
                        " are(is) sold");
                l_rr = l_rr + "    "+ p.getNum_sold() +" "+ p.getName() +
                        " are(is) sold" + "\n";
            }
    ...
    public String user_input(String s){
    ...
    public String staff_input(String s){
        List<String> ls = Arrays.asList(s.split(" "));
        String command = ls.get(0).toLowerCase();
